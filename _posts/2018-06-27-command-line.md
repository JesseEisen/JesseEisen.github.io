...
我们在日常编程中，对命令行参数的解析是不可避免的。一般情况下，不同的编程语言都提供了相应的库函数来解析命令行参数，比如 C 语言里面有 getopt 、getopt_long 这两个函数提供来解析命令行参数。 但是在我们着手使用这些工具或者函数前还是得了解一下命令行的基本格式。比如 GNU 的命令行参数的语法惯例如下：
